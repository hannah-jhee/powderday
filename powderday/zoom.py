...
def ramses_zoom(fname,ds,field_add):
    ...
    ledges = ds.arr(np.c_[reg[('index','x')] - reg[('index','dx')]/2, reg[('index','y')] - reg[('index','dy')]/2, reg[('index','z')] - reg[('index','dz')]/2], 'code_length')
    ds1.index.grid_left_edge = ledges
    ledges = ds.arr(np.c_[reg[('index','x')] + reg[('index','dx')]/2, reg[('index','y')] + reg[('index','dy')]/2, reg[('index','z')] + reg[('index','dz')]/2], 'code_length')
    ds1.index.grid_right_edge = ledges

    #ds1.index.grid_right_edge = reg[('index','x')] + reg[('index','dx')]/2
